...
- `frontend/` — Vite + React SPA. `src/main.jsx` (entry) → `App.jsx` → components (`DealGrid`, `DealCard`, `DealModal`, `FilterBar`, `TopBar`, `SubNav`, `Ticker`, `SourceLog`). `api.js` talks to the server.
...
## Retailer sources
- Zara + H&M: direct unofficial JSON endpoints (see scrapers.js header for endpoint notes).

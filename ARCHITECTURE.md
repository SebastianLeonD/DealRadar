...
- Shopify stores (`stores/shopify.js`, one generic `mapShopify`/`makeShopifyFetcher`): Gymshark (Clothing); Parachute, Coyuchi, Brooklinen, Our Place (Home); Mitzi, Color Cord, Vitruvi, Jonathan Y, Lulu and Georgia (For Your Room — lights/rugs/decor/air); JLab, Wyze, Mous, Epomaker, Sharge (Tech — audio/smart-home/cases/keyboards/charging) — each `/collections/{handle}/products.json`; a markdown = variant with `compare_at_price > price > 0`. Gymshark is men's-only via a `tags` filter (`Mens` and not `Womens`). Fetcher-set `category` sticks (runRefresh only fills it when absent), so "For Your Room" auto-appears as a nav tab. runRefresh also reroutes any `Home` deal whose title matches `categorize.isRoomItem` (desk/shelf/rug/lamp/mirror/…, bedding+kitchen words excluded) into "For Your Room", so IKEA desks/shelves/lamps land there too. Mixed price bands (user filters by price).

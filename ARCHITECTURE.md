...
- Shopify stores (`stores/shopify.js`, one generic `mapShopify`/`makeShopifyFetcher`): Gymshark (Clothing) + Parachute, Coyuchi, Brooklinen, Our Place (Home) — each `/collections/{handle}/products.json`; a markdown = variant with `compare_at_price > price > 0`. Gymshark is men's-only via a `tags` filter (`Mens` and not `Womens`). Home brands have mixed price bands (user filters by price).
